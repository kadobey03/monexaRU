    ...
    <div class="main-panel ">
        <div class="content ">
            <div class="page-inner">
                <div class="mt-2 mb-4">
                    <h1 class="title1  d-inline">Добавить пользователя в список рефералов {{ $user->name }}</h1>
                    <div class="d-inline">
                    ...
                    <div class="col-lg-8 offset-lg-2 card p-3  shadow">
                        <form method="POST" action="{{ route('addref') }}">
                            ...
                            <div class="form-group">
                                <h4 class="">Выбрать пользователя</h4>
                                <select class="form-control   select2" name="ref_id">
                                    @foreach ($ref as $item)
                                        <option value="{{ $item->id }}">{{ $item->name }}</option>
                                    @endforeach
                                </select>
                                <small class="">Это означает, что выбранный пользователь был приглашен
                                    {{ $user->name }}</small>
                            </div>
