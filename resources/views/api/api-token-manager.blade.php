<div>
    ...
    <x-jet-form-section submit="createApiToken">
        <x-slot name="title">
            {{ __('API Tokenu Oluştur') }}
        </x-slot>

        <x-slot name="description">
            {{ __('API tokenları, üçüncü taraf hizmetlerinin sizin adınıza uygulamamızda kimlik doğrulaması yapmasına izin verir.') }}
        </x-slot>
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-action-section>
                <x-slot name="title">
                    {{ __('API Tokenlarını Yönet') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Artık ihtiyaç duymuyorsanız mevcut tokenlarınızdan herhangi birini silebilirsiniz.') }}
                </x-slot>
    ...
    <x-jet-dialog-modal wire:model="displayingToken">
        ...
        <x-slot name="content">
            <div>
                {{ __('Lütfen yeni API tokenınızı kopyalayın. Güvenliğiniz için tekrar gösterilmeyecektir.') }}
            </div>
    ...
    <x-jet-confirmation-modal wire:model="confirmingApiTokenDeletion">
        ...
        <x-slot name="content">
            {{ __('Bu API tokenunu silmek istediğinizden emin misiniz?') }}
        </x-slot>
