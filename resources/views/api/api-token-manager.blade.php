<div>
    ...
    <x-jet-form-section submit="createApiToken">
        ...
        <x-slot name="description">
            {{ __('API токены позволяют третьим сторонам аутентифицироваться в нашем приложении от вашего имени.') }}
        </x-slot>
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-action-section>
                <x-slot name="title">
                    {{ __('Управление API токенами') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Вы можете удалить любой из существующих токенов, если больше не нуждаетесь в нём.') }}
                </x-slot>
                                ...
                                <div class="flex items-center">
                                    @if ($token->last_used_at)
                                        <div class="text-sm text-gray-400">
                                            {{ __('Последнее использование') }} {{ $token->last_used_at->diffForHumans() }}
                                        </div>
                                    @endif
    ...
    <x-jet-dialog-modal wire:model="displayingToken">
        ...
        <x-slot name="content">
            <div>
                {{ __('Пожалуйста, скопируйте ваш новый API токен. В целях безопасности он больше не будет отображён.') }}
            </div>
