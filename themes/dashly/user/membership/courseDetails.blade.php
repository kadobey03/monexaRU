            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="p-2 d-lg-flex justify-content-lg-between align-items-center mt-3">
                        ...
                        <div class="mt-2 mt-lg-0">
                            <p class="m-0 text-primary font-weight-bold">ПОСЛЕДНЕЕ ОБНОВЛЕНИЕ</p>
                                    ...
                                    <div>
                                        @if ($lesson->locked == 'true')
                                            <a href="#" data-bs-toggle="modal"
                                                ...
                                                class="btn btn-primary btn-sm">Превью</a>
                            ...
                            @if ($loop->iteration == 5)
                                <div>
                                    <div class="d-flex justify-content-between align-items-center mt-3">
                                        <div class="d-flex align-items-center justify-content-start">
                                            <div>
                                                <h6 class="h6 m-0">{{ $loop->remaining }} больше
                ...
                <div class="card-body">
                    @if (in_array(auth()->user()->id, $whoPurchased))
                        <a href="{{ route('user.mycoursedetails', $course->id) }}"
                            class="btn btn-primary btn-lg py-3 btn-block rounded-none rounded-0">Смотреть
                            урок</a>
                    @else
                    ...
                    <div class="modal-body">
                        <div class="text-center">
                            <p class="mb-3">
                                ...
                                будет списано с баланса вашего счета.
                            </p>
                            <form action="{{ route('user.buycourse') }}" method="post">
                                @csrf
                                <input type="hidden" name="amount" value="{{ $course->amount }}">
                                <input type="hidden" name="course" value="{{ $course->id }}">
                                <button type="submit" class="btn btn-primary btn-block">Купить сейчас</button>
                            </form>
                        </div>
                    </div>
                </div>
